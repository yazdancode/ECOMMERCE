...
class Cart:
    def __init__(self, request):
        ...
        cart = self.session.get(CART_SESSION_ID, {}).copy()  # کپی برای جلوگیری از تغییر در session
        self.cart = cart
    ...
    def __iter__(self):
        ...
        for item in cart_copy.values():
            ...
            item["price"] = Decimal(item["price"])  # استفاده از Decimal برای دقت بیشتر
    ...
    def add(self, product, quantity=1):
        ...
        if product_id not in self.cart:
            self.cart[product_id] = {
                "quantity": 0,
                "price": str(product.price),  # ذخیره قیمت به صورت string برای جلوگیری از مشکلات JSON
            }
